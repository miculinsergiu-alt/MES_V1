...
import { LayoutDashboard, FileText, Plus, X, Search, ChevronLeft, ChevronRight, Box, AlertCircle } from 'lucide-react';
...
function CreateOrderModal({ machines, onClose, onSave }) {
  ...
  // New state for Items and BOMs
  const [items, setItems] = useState([]);
  const [boms, setBoms] = useState([]);

  useEffect(() => {
    // Fetch items (ideally semi-finished and finished goods) and all BOMs
    Promise.all([
      api.get('/items'),
      api.get('/boms')
    ]).then(([iRes, bRes]) => {
      // Filter out raw materials if you only produce semi-finished and finished
  ...
  const setField = (i, k, v) => setOrders(p => p.map((o,idx) => {
    ...
    if (k === 'item_id') {
      ...
      newO.bom_id = ''; // reset BOM when item changes
    }
    return newO;
  }));
          ...
          {orders.map((order, i) => (
            <div key={i} style={{ background:'var(--bg-primary)', padding:16, borderRadius:'var(--radius-sm)', marginBottom:12, border:'1px solid var(--border)' }}>
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  <label className="form-label">Produs (Nomenclator)</label>
                  <select className="form-select" value={order.item_id} onChange={e=>setField(i,'item_id',e.target.value)} required>
              ...
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">BOM (Opțional)</label>
                  <select className="form-select" value={order.bom_id} onChange={e=>setField(i,'bom_id',e.target.value)} disabled={!order.item_id}>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        {materials.length > 0 && (
          <div style={{ marginTop:16 }}>
            <div style={{ fontSize:13, fontWeight:600, marginBottom:8 }}>Necesar Materiale (Conform BOM)</div>
            <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: 'var(--bg-primary)', textAlign: 'left' }}>
                  <th style={{ padding: '6px' }}>Poziție</th>
                  <th style={{ padding: '6px' }}>Articol</th>
                  <th style={{ padding: '6px' }}>Cantitate</th>
                  <th style={{ padding: '6px' }}>Locație</th>
                </tr>
              </thead>
              <tbody>
                {materials.map((m, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '6px' }}>{m.position_code}</td>
                    <td style={{ padding: '6px' }}>{m.item_code} - {m.item_name}</td>
                    <td style={{ padding: '6px', fontWeight: 600 }}>{m.required_quantity} {m.uom}</td>
                    <td style={{ padding: '6px' }}>{m.location || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
