...
## Jurnal Modificări (Sesiunea 1 Mai 2026)
- **Sistem BOM Multi-Nivel:**
  - **Database:** Extins `bom_positions` cu `parent_position_id`, `level` (1-4), `department` (magazie), `node_type` și `sort_order`.
  - **Backend Ierarhic:** Implementat rute `/api/boms/:id/tree` (recursiv) și salvări atomice prin ștergere/re-inserare recursivă a arborelui.
  - **Restructurare Vizualizare (BOMViewerPage):** 
    - Aliniere "cap la cap": Refactorizat `ViewerNode` folosind coloane flexibile cu lățimi fixe pentru Cantitate, Poziție și Cost, asigurând alinierea verticală perfectă indiferent de indentarea ierarhiei.
    - Navigare Inteligentă: Implementat `useLocation` state pentru a permite butonului "Editează BOM" să navigheze înapoi în `ItemsManager` și să deschidă automat modalul de editare corect.
  - **Sistem de Printare Profesional:** 
    - Creat layout special `print:block` tip tabel industrial (stil Excel) cu margini negre groase pentru lizibilitate în producție.
    - Implementat funcția `flattenForPrint` pentru a transforma arborele ierarhic într-o listă liniară indexată (ex: `..3`) potrivită pentru citirea rapidă de către operatori.
  - **Căutare Duală (Cod vs Denumire):** Implementat sistem de filtrare cu două câmpuri separate în `ItemsManager.jsx`. Permite căutarea simultană după "Cod Articol" și "Denumire/Descriere" pentru o navigare mult mai rapidă în liste mari de repere.
  - **Cost Standard:** Calcul automat recursiv al costului total al rețetei în timp real în ambele vederi (Editor și Viewer).
